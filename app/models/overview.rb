class Overview < ApplicationRecord
  ...
  with_options presence: true do
    ...
    validates :height, numericality: { greater_than_or_equal_to: 1, less_than_or_equal_to: 200 }, format: {with: /\A[0-9]+\z/},
               if: -> { physicalfinding_measuring_date.present? && weight.blank? && abdominal_circumference.blank? }
    validates :weight, numericality: { greater_than_or_equal_to: 1, less_than_or_equal_to: 200 }, format: {with: /\A[0-9]+\z/},
               if: -> { physicalfinding_measuring_date.present? && height.blank? && abdominal_circumference.blank? }
    validates :abdominal_circumference, numericality: { greater_than_or_equal_to: 1, less_than_or_equal_to: 200 }, format: {with: /\A[0-9]+\z/},
               if: -> { physicalfinding_measuring_date.present? && height.blank? && weight.blank? }
    ...
    validates :clinic_name,length: { maximum: 40 },
              if: -> { medeical_care_date.present? || disease_name.present? || treatment_medicine.present? }
    validates :disease_name,length: { maximum: 60 },
              if: -> { medeical_care_date.present? || clinic_name.present? || treatment_medicine.present? }
    validates :treatment_medicine,length: { maximum: 60 },
               if: -> { medeical_care_date.present? || clinic_name.present? || disease_name.present? }
  
    validates :vaccine_date,if: -> { physicalfinding_measuring_date.blank? && blood_urine_test_date.blank? && medeical_care_date.blank? },
                            if: :vaccine_type?
